...
function getPodsStatus() {
    ...
    if [ "${KUBECTL_VERSION}" == "v2" ]; then
        podStatus=$(kubectl get pods -n trustchain-fabric | grep "$1" | awk '{print $3}')
    else
        podStatus=$(kubectl get pods -n trustchain-fabric | grep "$1" | awk '{print $4}')
    fi
...
function getJobStatus() {
    if [ "${KUBECTL_VERSION}" == "v2" ]; then
        jobStatus=$(kubectl get jobs -n trustchain-fabric |grep "$1" | awk '{print $2}')
        if [ ${jobStatus} == "1/1" ]; then
            return 0
        fi
        return 1
    else
        jobStatus=$(kubectl get jobs -n trustchain-fabric |grep "$1" | awk '{print $3}')
        if [ ${jobStatus} == "1" ]; then
            return 0
        fi
        return 1
...
function checkJob() {
    ...
    getJobStatus ${job}
    while [ $? != "0" ]; do
        _pod=$(kubectl get pods -n trustchain-fabric --selector=job-name=$1 |sed '1d')
...
kubectl get jobs -n trustchain-fabric | sed '1d' | awk '{ print $1 }' | while read line; do
    kubectl delete jobs ${line} -n trustchain-fabric
done


applyBy 0copy-chaincode.yaml

pod=$(kubectl get pods -n trustchain-fabric --selector=job-name=copychaincode --output=jsonpath={.items..metadata.name})
podSTATUS=$(kubectl get pods -n trustchain-fabric --selector=job-name=copychaincode --output=jsonpath={.items..phase})
...
kubectl cp -n trustchain-fabric ./chaincode/${CHAINCODE_NAME} $pod:/shared/artifacts/chaincode/
